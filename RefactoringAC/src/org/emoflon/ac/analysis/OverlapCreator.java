 ...
 *
 *         This class is responsible for creating rule and pattern overlaps,
 *         thus, identifiying common parts
 */
public class OverlapCreator {
	...
	/**
	 * This method is used to register overlaps, so that an overlap can be derived
	 * using an {@link IBeXGTMatch}
	 * 
	...
	public void registerOverlap(IBeXGTPattern pattern, String... parameters) {
		...
		else if (overlapSize != parameters.length)
			throw new RuntimeException("The registered overlap for pattern " + patternName + " has " + parameters.length
					+ " parameters but was expected to have " + overlapSize);
