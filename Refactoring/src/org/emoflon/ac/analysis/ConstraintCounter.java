 ...
 *
 *         This class is reponsible for counting repair-indicating and
 *         violation-indicating application condition matches and mapping them
...
public class ConstraintCounter {
	...
	public ConstraintCounter(Collection<OverlapCreator> violationOverlapCreators,
			Collection<OverlapCreator> repairOverlapCreators) {
	...
	public void addViolation(IBeXGTMatch match) {
		...
		for (var violationOverlapCreator : violationOverlapCreators) {
			...
			var violations = creator2violations.get(violationOverlapCreator).overlap2matches().computeIfAbsent(overlap,
					(x) -> new HashSet<>());
	...
	public void addRepair(IBeXGTMatch match) {
		...
		for (var repairOverlapCreator : repairOverlapCreators) {
			...
			var repairs = creator2repairs.get(repairOverlapCreator).overlap2matches().computeIfAbsent(overlap,
					(x) -> new HashSet<>());
			repairs.add(match);
